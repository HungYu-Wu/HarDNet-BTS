...
from loss import EDiceLoss, EDiceLoss2
...
parser.add_argument('-a', '--arch', metavar='ARCH', default='hardnet4',
...
parser.add_argument('--optim', choices=['ranger', 'sgd', 'ranger', 'adamw','ranger21'], default='ranger')
...
def main(args):
    ...
    model = model_maker(deep_supervision=args.deep_sup)
    print(f"total number of trainable parameters {count_parameters(model)}")

    if args.swa:
        # Create the average model
        swa_model = model_maker(deep_supervision=args.deep_sup)
    ...
    if ngpus > 1:
        ...
        #weights = torch.load('/home/a9778875/BraTs2020/src/runs/17/model_best.pth.tar')
        #model.load_state_dict(weights['state_dict'])
    else:
        model = model.cuda()
        #weights = torch.load('/home/a9778875/BraTs2020/src/runs/003/model_best.pth.tar')
        #model.load_state_dict(weights['state_dict'])
    ...
    if args.optim == "adam":
        ...
        optimizer = torch.optim.SGD(model.parameters(), lr=1e-2, weight_decay=1e-4, momentum=0.99,
                                    nesterov=True)

    elif args.optim == "adamw":
        print(f"weight decay argument will not be used. Default is 11e-2")
        optimizer = torch.optim.AdamW(model.parameters(), lr=1e-4, weight_decay=0.01, eps=1e-4, amsgrad=True)
    ...
    if args.resume:
        reload_ckpt(args, model, optimizer, scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer,round(args.epochs * 0.5)))
    ...
    if args.full:
        ...
        train_loader = torch.utils.data.DataLoader(
            train_dataset, batch_size=args.batch_size, shuffle=True,
            num_workers=args.workers, pin_memory=False, drop_last=True)#,collate_fn=determinist_collate)

        val_loader = torch.utils.data.DataLoader(
            val_dataset, batch_size=max(1, args.batch_size // 2)
            , shuffle=False, pin_memory=False, num_workers=args.workers, collate_fn=determinist_collate)
       
        bench_loader = torch.utils.data.DataLoader(
            bench_dataset, batch_size=1, num_workers=args.workers)
        print("Val dataset number of batch:", len(val_loader))
        print("bench dataset number of batch:", len(bench_loader))
    ...
    if args.warm_restart:
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer,round(args.epochs * 0.5))
    print("start training now!")
    if args.swa:
        # c = 15, k=3, repeat = 5
        c, k, repeat = 30, 1, args.swa_repeat
    ...
    if args.swa:
        ...
        for i in range(repeat):
            optimizer =Ranger(model.parameters(), lr=args.lr / 2)
            ...
            for swa_epoch in range(c):
                ...
                save_folder=args.save_folder, count = count)
                count = count+1
    ...
    try:
        ...
        generate_segmentations(bench_loader, model)
    except KeyboardInterrupt:

...
def save_checkpoint1(state: dict, save_folder: pathlib.Path):
    """Save Training state."""
    best_filename = f'{str(save_folder)}/model_fin.pth.tar'
    torch.save(state, best_filename)


def save_checkpoint2(state: dict, save_folder: pathlib.Path,count = int):
    """Save Training state."""
    best_filename = f'{str(save_folder)}/model_swa{str(count)}.pth.tar'
    torch.save(state, best_filename)
...
def reload_ckpt(args, model, optimizer, scheduler=None):
    ...
    if os.path.isfile(ckpt):
        ...
        try:
            ...
            for k,v in model.state_dict().items():
                if v.shape:
                    keys.append(v.data.cpu())
            df = pd.DataFrame(keys)
            df.to_csv((str('./preds/best')+'/'+'weight1.csv'), index=False)
                #print(k,v)
            '''
            if optimizer:
                optimizer.load_state_dict(checkpoint['optimizer'])
...
# +
def generate_segmentations(data_loader, model):
    metrics_list = []
    for i, batch in enumerate(data_loader):
        ...
        inputs1 = inputs.cuda().cpu()
        inputs1 = inputs1.numpy()
        df1 = pd.DataFrame(inputs1[0][0][70])
        df1.to_csv((str('./preds/best')+'/'+'results1.csv'), index=False)
        with autocast():
            with torch.no_grad():
                if model.module.deep_supervision:
                    ...
                    pre_segs1 = pre_segs.cuda().cpu()
                    inputs1 = pre_segs1.detach().numpy()
                    df1 = pd.DataFrame(inputs1[0][0][70])
                    df1.to_csv((str('./preds/best')+'/'+'pre1.csv'), index=False)
...
# -
    ...
    '''
def generate_segmentations1(data_loader, model):
    metrics_list = []
    for i, batch in enumerate(data_loader):
        # measure data loading time
        inputs = batch["image"]
        ...
        # remove pads
        #print(inputs.shape, pre_segs.shape)
        maxz, maxy, maxx = pre_segs.size(2) - pads[0], pre_segs.size(3) - pads[1], pre_segs.size(4) - pads[2]
        pre_segs = pre_segs[:, :, 0:maxz, 0:maxy, 0:maxx].cpu()
        segs = torch.zeros((1, 3, 155, 240, 240))
